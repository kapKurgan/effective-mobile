...
from pages.site_pages import LOGO_PAGE_PRODUCTS, VALUE_BUTTON_LOGIN
from utils.checks import attach_screenshot, expect_count, expect_visible
...
def _open_burger_menu(login_page):
    ...
    """
    expect_visible(login_page.button_product_bm_open, "Кнопка 'Открыть бургер меню'")
...
def _collect_button_data(buttons_product_bm_list):
    ...
    with allure.step("Cобрать названия всех кнопок 'Бургер меню' в список"):
        ...
        expect_count(buttons_product_bm_list, 4)
...
def _close_burger_menu(login_page):
    ...
    """
    expect_visible(login_page.button_product_bm_close, "Кнопка 'Закрыть бургер меню'")
